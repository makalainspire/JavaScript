...
function validateRequired() {
    ...
    try {
        for (var i = 0; i < elementCount; i++) {
            //validate all required input elements in field
        ...
        if (!crustBoxes[0].checked && !crustBoxes[1].checked) {
            //verify that crust is selected
            crustBoxes[0].style.outline = "1px solid red";
            crustBoxes[1].style.outline = "1px solid red";
            fieldsetValidity = false;
        } else {
            crustBoxes[0].style.border = "";
            crustBoxes[1].style.border = "";
        }
        if (fieldsetValidity === false) {
            throw "Please complete all required feilds.";
        } else {
...
function validateForm(evt) {
    ...
    if (formValidity === true) {
        ...
        document.getElementById("errorMessage").innerHTML = "Please complet the highlighted fields";
